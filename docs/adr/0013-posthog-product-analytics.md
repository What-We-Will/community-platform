...
**The ingest proxy builds a fresh outbound request; cookies never reach PostHog.** Because the proxy is same-origin and every app cookie (including Supabase session tokens) is set with `path=/`, the browser attaches all of them to every `/ingest/*` request. A config-level Next.js rewrite — PostHog's documented setup — relays the full request, so a PostHog data breach, subpoena, or insider incident would expose live session credentials, not just pseudonymous analytics. The route handler therefore forwards an explicit header allowlist (`content-type`, `user-agent`) and nothing else: no `Cookie`, no `Authorization`, and no client IP header — PostHog sees only the Vercel egress IP, making the IP-discard posture below code-enforced rather than dashboard-dependent. The handler also confines the upstream URL to the two PostHog hosts (returning 400 on any mismatch), so the public ingest endpoint cannot be re-targeted as an open proxy. Accepted cost of deviating from the documented rewrite pattern: each ingest request invokes a serverless function rather than being relayed at Vercel's routing layer — marginal latency and invocation cost on the analytics path only, bounded by the firewall rate limits above — plus a small proxy module to own instead of a config block. **This deviation was reviewed and signed off by the named approver (2026-08-31).** Widening the forwarded-header list is a change to this ADR, not a code tweak.
...
**Capture scope is an allowlist, not a deny-list.** Only routes in the authenticated member app may emit events. Login, signup, onboarding, password recovery, pending-approval (except its logout reset), and the entire public marketing site emit nothing, enforced in code (`before_send` route gate, fail closed). A deny-list of named routes is rejected because it silently captures every route nobody thought to name. Known residual, **accepted by the named approver (2026-08-31)**: the SDK itself initializes globally whenever the token is set, so excluded routes still get a persistent anonymous device identifier (localStorage+cookie) and remote-config/flags requests even though no events leave them and `identified_only` creates no person profile for them. Route-scoped initialization would close this but is deferred as a separate decision — and is not free: the login-page lingering-identity reset (below) requires the SDK to be loaded on the login page, an excluded route, so fully route-scoping init would break that control unless special-cased. Cost accepted: no signup-funnel or public-site analytics; either would require its own consent posture and a new ADR.
...
**DPA and subprocessors.** PostHog's Data Processing Agreement is executed (via its standard terms) and reviewed by the platform lead before production capture is enabled, covering the pseudonymous personal data described above. PostHog's subprocessor list is reviewed at the same time and re-checked as part of the quarterly review; a subprocessor change that would move member data outside the accepted posture blocks capture until resolved or accepted by amendment here.
...
- Recurring operational work is created: quarterly seat + dashboard-settings + subprocessor-list review; a +2-weeks post-enable event-volume review (autocapture runs 5–10× custom-event volume; dials are pageleave capture and autocapture allowlists); deletion/export handling per above.
