...
import { CancelHelpButton } from "./CancelHelpButton";
...
export async function InterviewHelpCard({ currentUserId }: { currentUserId: string }) {
  ...
  const othersCount = requests.filter((r) => r.user_id !== currentUserId).length;
  ...
  return (
    <Card>
      ...
      <CardContent>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="text-sm font-medium leading-snug truncate">
                          {isOwn ? "You need help" : `${req.requester?.display_name ?? "A member"} needs help`}
                        </p>
                        {isOwn && (
                          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200 shrink-0">
                            Your request
                          </span>
                        )}
                      </div>
                    ...
                    {isOwn ? (
                      <CancelHelpButton helpId={req.id} />
                    ) : req.requester && (
        ...
        {othersCount === 0 && requests.length > 0 && (
          <p className="text-xs text-muted-foreground text-center mt-3">
            No other members have open requests yet.
          </p>
        )}
      </CardContent>
    </Card>
