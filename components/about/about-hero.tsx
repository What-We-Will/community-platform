export function AboutHero() {
  return (
    <section className="relative overflow-hidden bg-dark-blue px-4 py-16 text-white md:py-24">
      <div className="mx-auto max-w-3xl text-center">
        <h1 className="font-bebas text-4xl tracking-wide text-white sm:text-5xl md:text-6xl">
          About Us
        </h1>
        <p className="mt-4 text-lg leading-relaxed text-white/90 sm:text-xl">
          We are a community of knowledge workers, organizers, and advocates mobilizing
          to win an equitable and dignified future of work.
        </p>
      </div>
    </section>
  );
}
