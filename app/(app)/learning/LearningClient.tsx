...
import {
  ...
  ListTodo, Users2, Check, ChevronLeft, ChevronRight as ChevronRightIcon,
  X,
} from "lucide-react";
...
import type { TrackerItem, StudyGroupRow } from "./page";
...
function TrackerKanbanCard({ item }: { item: TrackerItem }) {
  ...
  const typeIcon = item.resource?.type === "video"
    ? <PlaySquare className="size-3 shrink-0" />
...
function MyTrackerTab({ trackerItems }: MyTrackerTabProps) {
  return (
    <div className="space-y-4">
      {trackerItems.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-xs">
            Browse Courses, Videos, or Tutorials and click <strong>Add to My Tracker</strong> to get started.
          </p>
        </div>
      )}
...
export function LearningClient({
  ...
  trackerItems, trackerByResource, studyGroupsByResource,
}: Props) {
      ...
      <Tabs defaultValue="paths">
        <TabsList className="mb-4 flex-wrap h-auto gap-1">
          ...
          <TabsTrigger value="tracker" className="gap-1.5">
            <ListTodo className="size-3.5" /> My Tracker
            {trackerItems.length > 0 && (
              <span className="ml-1 rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                {trackerItems.length}
              </span>
        ...
        <TabsContent value="tracker">
          <MyTrackerTab trackerItems={trackerItems} currentUserId={currentUserId} />
        </TabsContent>
      </Tabs>
    </div>
  );
